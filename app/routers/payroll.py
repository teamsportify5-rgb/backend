...
from app.models import User, Attendance, Payroll
...
async def generate_payroll(
    ...
    if payroll_data.basic_salary is not None:
        ...
        # Auto-calculate based on employee's daily rate or default rates
        if employee.daily_rate and employee.daily_rate > 0:
            daily_rate = employee.daily_rate
        else:
            # Default daily rates by role (fallback)
            daily_rates = {
                "admin": 5000,
                "manager": 4000,
                "accountant": 3000,
    ...
    # Calculate deductions (use manual override if provided, otherwise 10% of basic salary)
    if payroll_data.deductions is not None:
        deductions = payroll_data.deductions
    else:
        deductions = basic_salary * 0.1
    
    # Calculate bonus (use manual override if provided, otherwise 5% if attendance > 25 days)
...
async def get_all_payroll_records(
    ...
    # Query all payroll records, ordered by month (descending) and employee_id
    payroll_records = db.query(Payroll).order_by(
        Payroll.month.desc(),
        Payroll.employee_id
...
async def get_payroll_records(
    ...
    payroll_records = db.query(Payroll).filter(
        Payroll.employee_id == employee_id
    ).offset(skip).limit(limit).order_by(Payroll.month.desc()).all()
...
async def get_payroll_slip(
    ...
    emp_data = [
        ...
        ['Month:', payroll.month],
        ['Generated Date:', payroll.generated_at.strftime('%Y-%m-%d %H:%M:%S')]
    ]
